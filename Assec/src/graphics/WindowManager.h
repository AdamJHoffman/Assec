...
namespace assec::graphics
{
	...
	public:
		~WindowManager();
		static void init(const ref<WindowContext> windowContext, std::function<void(events::Event*)> eventCallBackFn);
		static void prepare();
		static void finish();
		const static bool empty();
		const static void clear();
		const static Window& addWindow(uint32_t width, uint32_t height, const char* title, Monitor* monitor, Window* share);
