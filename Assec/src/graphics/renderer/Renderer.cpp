	...
	void Renderer::endScene()
	{
		...
		for (auto key : util::Keys(m_BatchManager.getBatches()))
		{
			...
			for (auto b : (m_BatchManager.getBatches()).at(key))
			{
				...
				m_VertexArray->m_VertexBuffer->addSubData(&b.createMesh()->createVerticesData()[0], sizeof(float) * b.createMesh()->createVerticesData().size(), 0);
				m_VertexArray->m_IndexBuffer->addSubData(&b.createMesh()->getIndices()[0], sizeof(uint32_t) * b.createMesh()->getIndices().size(), 0);
